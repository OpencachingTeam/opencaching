	                   ...
	                   INNER JOIN `cache_status` ON `caches`.`status`=`cache_status`.`id`
	                        WHERE `caches`.`latitude`>'&1' AND 
					             ...
					             ORDER BY `distance` ASC LIMIT 1",
										            geomath::getMinLat($nLon, $nLat, $nMaxDistance),
										            geomath::getMaxLat($nLon, $nLat, $nMaxDistance),
										            geomath::getMinLon($nLon, $nLat, $nMaxDistance),
										            geomath::getMaxLon($nLon, $nLat, $nMaxDistance),
										            $login->userid);
