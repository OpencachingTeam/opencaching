		...
		<tr>
			<td colspan="2">
				<table class="null" border="0" cellspacing="0">
					<tr>
						<td class="header-small">{t}Name{/t}</td>
						<td class="header-small" width="300px">{t}Download{/t}</td>
						<td class="header-small" width="100px">&nbsp;</td>
					</tr>
					{foreach from=$queries item=queriesItem}
						...
						<tr>
							<td colspan="2" bgcolor="{$bgcolor}">&nbsp;&nbsp;&nbsp;&nbsp;</td>
							<td bgcolor="{$bgcolor}">[<a href="javascript:if(confirm('{t escape=js}Do you really want to delete the saved search?{/t}'))location.href='query.php?queryid={$queriesItem.id}&action=delete'">{t}Delete{/t}</a>]</td>
						</tr> 
